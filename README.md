...
### 🧬 Polypharmacy Side Effect Prediction via LLMs
`ChemBERTa` `RDKit` `GNN` `PyTorch` `Multi-label Classification`
...
`CNN` `VGG16` `Transfer Learning` `Computer Vision`
...
### 🖼️ Face Restoration in Degraded Images
`GANs` `Image Restoration` `IEEE 2022` `Research Reproduction`
